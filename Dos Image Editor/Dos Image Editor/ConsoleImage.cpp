...
#include <vector>		// new version of arrays
#include "Console.h"
...
ConsoleImage::ConsoleImage(Console &parent, ConsoleImage *source, int frame, int x1, int y1, int x2, int y2)
{
	...
	for(int yCount = 0; yCount < imageSize.Y; ++yCount)
	{
		for(int xCount = 0; xCount < imageSize.X; ++xCount)
		{
			...
			copyspot = ((yCount+y1)*imageSize.X)+xCount+x1;
...
void ConsoleImage::Draw(int frame, int x, int y)
{
	COORD pos;
	int spot;
	for(int yCount = 0; yCount < imageSize.Y; ++yCount)
		...
		if ((y+yCount < Parent->GetBufHeight()) && (y+yCount >= 0))
		{
			for(int xCount = 0; xCount < imageSize.X; ++xCount)
			{
				if ((x+xCount < Parent->GetBufWidth()) && (x+xCount >= 0))
				{
					...
					spot = (imageSize.X*imageSize.Y*frame)+(yCount*imageSize.X)+xCount;
...
int ConsoleImage::GetWidth()
{
	return imageSize.X;
}

// gets the height of the image
int ConsoleImage::GetHeight()
{
	return imageSize.Y;
}

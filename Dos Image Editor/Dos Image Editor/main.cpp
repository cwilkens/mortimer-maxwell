		...
		if (timer_dif > 0.05)
		{
			...
			if (Con.MouseArea(0,1,78,37) && next.all == false)
			{
				...
				if (mode == "Frame")
				{
					...
					{
						Key.up = false;
						if (imageBuffer.GetHeight() > 1)
							imageBuffer.SetHeight(imageBuffer.GetHeight()-1);
					} 
					...
					{
						Key.left = false;
						if (imageBuffer.GetWidth() > 1)
							imageBuffer.SetWidth(imageBuffer.GetWidth()-1);
					} 
			...
			if (Con.GetKey(VK_LBUTTON))
			{
				if ((cur.X == 0) && (cur.Y == 38) && (sX > 0))
					sX -= 1;
				if ((cur.X == 78) && (cur.Y == 38) && (sX < imageBuffer.GetWidth()-78))
					sX += 1;
				if ((cur.X == 79) && (cur.Y == 1) && (sY > 0))
					sY -= 1;
				if ((cur.X == 79) && (cur.Y == 37) && (sY < imageBuffer.GetHeight()-36))
					sY += 1;
			}
			...
			Con.SetColor(imageBuffer.GetWidth()-sX,1,C_GRAY,C_BLACK,1,imageBuffer.GetHeight()-sY);
			Con.WriteChar(imageBuffer.GetWidth()-sX,1,'\xDD',1,imageBuffer.GetHeight()-sY);
			Con.SetColor(0,imageBuffer.GetHeight()+1-sY,C_GRAY,C_BLACK,imageBuffer.GetWidth()-sX,1);
			Con.WriteChar(0,imageBuffer.GetHeight()+1-sY,'\xDF',imageBuffer.GetWidth()-sX,1);
			...
			float scale = (float)(sX+(sX/(float)(imageBuffer.GetWidth()-78))*77)/imageBuffer.GetWidth();
			Con.WriteChar(1+(int)(scale*77),38,'\xDB');
			scale = (float)(sY+(sY/(float)(imageBuffer.GetHeight()-36))*35)/imageBuffer.GetHeight();
			...
			Con.WriteStr(55,0,Con.IntToStr(imageBuffer.GetWidth()).append(",").append(
				Con.IntToStr(imageBuffer.GetHeight())));

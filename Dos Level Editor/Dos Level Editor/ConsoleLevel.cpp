...
{
	int spot;
	for(int yCount = y; yCount < y+source->GetHeight(); ++yCount)
	{
		if ((yCount >= 0) && (yCount < levelSize.Y))
		{
			for(int xCount = x; xCount < x+source->GetWidth(); ++xCount)
			{
...
// adds an object to the level data
...
// removes an object from the level data
void ConsoleLevel::RemoveObject(int index)
{
...
void ConsoleLevel::Load(std::string filename)
{
	...
	if (image_file.is_open())
	{
		...
		while (image_file.peek() != EOF)
		{
			...
			std::string file = copyobj.imagename;
			file.append(".dat");
			// load it
			copyobj.image = new ConsoleImage(*Parent, "data/objects/"+file);
			copyobj.image->SetTransparent(C_BLACK);

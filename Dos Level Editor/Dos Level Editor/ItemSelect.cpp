...
int ItemSelect::Step()
{
	...
	if (menu_down != -1)
	{
		...
		for(WORD menu_c = 0; menu_c < menu_pics[menu_down]->GetWidth(); ++menu_c)
		{
			for(WORD menu_c2 = 0; menu_c2 < menu_pics[menu_down]->GetHeight(); ++menu_c2)
			{
				if (Parent->MouseArea(x_menu[menu_down]+menu_c,
					position.Y-menu_pics[menu_down]->GetHeight()+menu_c2,
					x_menu[menu_down]+menu_c,
					position.Y-menu_pics[menu_down]->GetHeight()+menu_c2) == true)
				{
...
void ItemSelect::Draw()
{
	...
	if (menu_down != -1)
	{
		menu_pics[menu_down]->Draw(0,x_menu[menu_down],
			position.Y-menu_pics[menu_down]->GetHeight());
		
		for(int yCount = position.Y-menu_pics[menu_down]->GetHeight(); 
			yCount < position.Y; ++yCount)
		{
			...
			Parent->SetColor(x_menu[menu_down]+menu_pics[menu_down]->GetWidth(),
				yCount,C_WHITE,C_BLUE);
			Parent->WriteChar(x_menu[menu_down]+menu_pics[menu_down]->GetWidth(),
				yCount,'\xBA');
		}
		Parent->SetColor(x_menu[menu_down]-1,position.Y-menu_pics[menu_down]->GetHeight()-1,
			C_WHITE,C_BLUE,menu_pics[menu_down]->GetWidth()+2);
		Parent->WriteChar(x_menu[menu_down],position.Y-menu_pics[menu_down]->GetHeight()-1,
			'\xCD',menu_pics[menu_down]->GetWidth());
		Parent->WriteChar(x_menu[menu_down]-1,
			position.Y-menu_pics[menu_down]->GetHeight()-1,'\xC9');
		Parent->WriteChar(x_menu[menu_down]+menu_pics[menu_down]->GetWidth(),
			position.Y-menu_pics[menu_down]->GetHeight()-1,'\xBB');
	}
}

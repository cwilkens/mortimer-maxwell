...
Console::Console(int width, int height)
{
	...
	{
		//bufConsole[bCount].Char.UnicodeChar = 0;
...
void Console::SetFColor(int _x, int _y, int color, int width, int height)
{
	int spot;
	for(int yCount = 0; yCount < height; ++yCount)
	{
		if ((_y+yCount < size.Y) && (_y+yCount >= 0))
		{
			for(int xCount = 0; xCount < width; ++xCount)
			{
				if ((_x+xCount < size.X) && (_x+xCount >= 0))
				{
					// calculate the spot and the color
					spot = size.X*_y + size.X*yCount;
					bufConsole[spot+_x+xCount].Attributes = color +
						(bufConsole[spot+_x+xCount].Attributes/16)*16;
				}
			}
		}
	}
...
// sets the background color in one cell
void Console::SetBColor(int _x, int _y, int color, int width, int height)
{
	int spot;
	for(int yCount = 0; yCount < height; ++yCount)
	{
		if ((_y+yCount < size.Y) && (_y+yCount >= 0))
		{
			for(int xCount = 0; xCount < width; ++xCount)
			{
				if ((_x+xCount < size.X) && (_x+xCount >= 0))
				{
					// calculate the spot and the color
					spot = size.X*_y + size.X*yCount;
					bufConsole[spot+_x+xCount].Attributes =
						(bufConsole[spot+_x+xCount].Attributes/16) + color*16;
				}
			}
		}
	}
...
void Console::WriteStr(int _x, int _y, std::string str1)
{
	...
	for(int sCount = 0; sCount < (int)str1.length(); ++sCount)
	{
		if ((_x >= 0) && (_y >= 0) && (_x+sCount < size.X) && (_y < size.Y))
			bufConsole[size.X*_y+_x+sCount].Char.AsciiChar = str1.at(sCount);
	}
}
...
// returns if the mouse is in the given area
bool Console::MouseZone(int x, int y, int width, int height)
{
	int x1 = x+width-1;
	int y1 = y+height-1;
	if ((mouse.x >= x) && (mouse.x <= x1) && (mouse.y >= y) && (mouse.y <= y1))
		return true;
	else
		return false;
}
